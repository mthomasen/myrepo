...
```{r}
pacman::p_load(tidyverse, brms, ggplot, rstanarm)
...
### plot simulated data
```{r}
plot1 <- ggplot(df, aes(visit,MLU_true, color=diagnosis, group=ID))+
  theme_bw()+
  geom_point()+
  geom_line(alpha=0.3)
...
plot2 <- df %>% 
  ...
  ggplot(aes(visit,MLU_true, group=ID, color="ASD"))+
  theme_bw()+
  geom_point()+
  geom_line(alpha=0.3)
...
plot3 <- df %>% 
  filter(diagnosis=="TD") %>% 
  ggplot(aes(visit,MLU_true, group=ID, color="TD"))+
  theme_bw()+
  geom_point()+
  geom_line(alpha=0.3)
...
```{r}
MLU_f4 <- bf(MLU_true ~ 0 + diagnosis + diagnosis:visit + (1 + visit| gr(ID, by = diagnosis)))

```
priors
```{r}

priors <- c(prior(normal(0,0.1),class=b, lb=0),
...
pp_check(MLU_p4_prior, ndraws=100)
```
