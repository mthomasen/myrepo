...
demo_data <- rename(demo_data, id=Child.ID)

lu_data <- rename(lu_data, visit=VISIT)
word_data <- rename(word_data, visit=VISIT)
demo_data <- rename(demo_data, visit=Visit)
...
```{r}
lu_data$id <- lu_data$id %>% 
  str_replace_all('[.]','')

demo_data$id <- demo_data$id %>% 
  str_replace_all('[.]','')

word_data$id <- word_data$id %>% 
  str_replace_all('[.]','')
...
```{r}
demo_data <- rename(demo_data, non_verbalIQ=MullenRaw)

demo_data <- rename(demo_data, verbalIQ=ExpressiveLangRaw)
```
...
```{r}
df_demo <- merge(lu_data, word_data)

df <- merge(df_demo, demo_data)
...
df <- df %>% 
  select('id', 'visit','Diagnosis','Ethnicity','Gender','Age','ADOS','non_verbalIQ','verbalIQ','Socialization','MOT_MLU','CHI_MLU','types_MOT','types_CHI','tokens_MOT', 'tokens_CHI')
...
```{r}
visit1_df <- df %>% 
  select('id', 'visit', 'ADOS', 'non_verbalIQ', 'verbalIQ', 'Socialization')%>%
  filter(visit==1)

visit1_df <- rename(visit1_df, ADOS1=ADOS) 

visit1_df <- rename(visit1_df, non_verbalIQ1=non_verbalIQ) 
visit1_df <- rename(visit1_df, verbalIQ1=verbalIQ) 

visit1_df <- rename(visit1_df, Socialization1=Socialization) 

visit1_df <- visit1_df %>% 
    select('id', 'ADOS1', 'non_verbalIQ1', 'verbalIQ1', 'Socialization1')

final_df <- merge(visit1_df,df)
```
...
_I already did that earlier in the script_
...
```{r}
#final_df$Gender <- final_df$Gender %>% 
  #str_replace_all('[1]','female')

#final_df$Gender <- final_df$Gender %>% 
 # str_replace_all('[2]','male')

final_df$Gender <- final_df$Gender %>% 
  recode_factor('1'='female','2'='male')

write_csv(final_df,'assignment_0_data.csv')
